...
describe("Connexion utilisateur", () => {
  ...
  it("SC07 - devrait afficher la page de connexion avec les champs requis", () => {
    ...
    // Attention : le "i" de "in" est en minuscule dans l'app !
  ...
  it("SC11 - devrait afficher une erreur et désactiver le bouton si les champs sont vides", () => {
    // On clique sur "Sign In" sans rien remplir
    cy.getBySel("signin-submit").click();

    // Le message d'erreur "Username is required" apparaît sous le champ
    cy.get("#username-helper-text")
      .should("be.visible")
      .and("contain", "Username is required");

    // Après la validation, le bouton devient disabled (grisé)
    cy.getBySel("signin-submit").should("be.disabled");

    // On tape le username → toujours disabled (il manque le password)
    cy.getBySel("signin-username").type("Heath93");
    cy.getBySel("signin-submit").should("be.disabled");
  ...
  it("SC12 - devrait afficher une erreur si le username est effacé après saisie", () => {
    // .find('input') → on cible le vrai champ <input> dans le composant Material UI
    // Sans ça, Cypress essaie de taper dans le <div> wrapper et ça échoue
    cy.getBySel("signin-username").find("input").type("test").clear();
